...
ROSPixelCloudFusionApp::TransformPoint(const pcl::PointXYZ &in_point, const tf::StampedTransform &in_transform)
{
  ...
  //ROS_INFO_STREAM("Input points:=" << "\t" << in_point.x << "\t" << in_point.y << "\t" << in_point.z);

 // ROS_INFO_STREAM("Transformed points:=" << "\t" << tf_point_t.x() << "\t" << tf_point_t.y() << "\t" << tf_point_t.z());

  return pcl::PointXYZ(tf_point_t.z(),tf_point_t.y(), tf_point_t.x());
}
...
void ROSPixelCloudFusionApp::CloudCallback(const sensor_msgs::PointCloud2::ConstPtr &in_cloud_msg)
{
  ...
  for (size_t i = 0; i < in_cloud->points.size(); i++)
  {
    ...
    */
    ...
    //ROS_INFO_STREAM("X := "<< u << "\t" <<"Y :=" << v);

    //log_files <<cam_cloud[i].x<<","<<cam_cloud[i].y<<","<<cam_cloud[i].z<<"|"<<u<<","<<v<<"\n"; 
